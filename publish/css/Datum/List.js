...
function () {
  function _default(args) {
    ...
    this.setValue(value, _types.WITH_OUT_DISPATCH);
  ...
  _proto.setValue = function setValue(values, type) {
    ...
    if ((0, _deepEql.default)(values, this.$values)) return;

    if (type === _types.WITH_OUT_DISPATCH) {
      this.$values = this.formatValue(values);
      this.resetValueMap();
    } else {
      this.resetValue(this.formatValue(values), (0, _shallowEqual.default)(this.$cachedValue, values));
    }
