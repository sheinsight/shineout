...
export var getFilterTree = function getFilterTree(treeNodes, filterFunc, filterExpandKeys, keyFunc, childrenKey, showHitDescendants, firstMatchNode, _temp) {
  ...
  var mapFilteredNodeToData = function mapFilteredNodeToData(node) {
    ...
    if (children.length || match) {
      ...
      return _objectSpread({}, node, (_objectSpread2 = {}, _objectSpread2[childrenKey] = childNodes, _objectSpread2));
    }
...
export var getFlattenTree = function getFlattenTree(data, childrenKey) {
  if (childrenKey === void 0) {
    childrenKey = 'children';
  }
...
export var mergeFilteredTree = function mergeFilteredTree(filterDatum, rawDatum, tiledId) {
  ...
  return immer(filterData, function (draft) {
    draft.map(recursion);
  });
};