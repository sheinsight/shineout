...
### 2.0.32
- `Form` 新增 `deepClone` 属性，支持关闭表单值的深克隆以提升大数据量或复杂对象场景下的性能
- `Form` 新增 `immutable` 属性，以基于路径的局部浅拷贝更新表单值，避免 `set` 原地修改污染共享引用；`deepClone` 为 `false` 时自动开启，传 `immutable={false}` 可显式关闭
...
- 修复 `Table` 的固定列遇到合并列时阴影显示异常的问题
- 修复弹窗类组件上报的埋点错误
- 修复 `TreeSelect` 组件 value 为数字 0 时不回填的问题


### 2.0.30

- 修复 `Dropdown` 的触发方式为 hover 时弹出层快速消失的问题（Regression: since 2.0.29） (< 2.0.30)
- 修复 `DatePicker` 在受控模式下，外部 value 变化后组件的结果可能不会重新渲染的问题 (< 2.0.30)
- 修复 `Popover` 在微前端场景下可能出现箭头样式异常的问题 (< 2.0.30)
