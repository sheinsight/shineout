# 更新日志
### 1.12.13-beta.9
- 修复 Input.Number 在失焦时触发的 onChange 会变成string类型的问题(< 1.12.13)

### 1.12.13-beta.8
- 修复 Table 样式权重问题 (1.8.8 - 1.12.12)
- 修复 Table 内部 sorter 对象导致重复渲染的问题 (< 1.12.13)
- 修复 Select value = 0 的时候 result 不渲染的问题 (< 1.12.13)

### 1.12.13-beta.7
- 修复 Select 下拉框弹出位置计算偏差导致菜单弹出错误的问题 (< 1.12.13)

### 1.12.13-beta.6
- 修复 Popover 受控时无法加载的问题(< 1.12.13)

### 1.12.13-beta.5
- 修复 cascader filter 时候动态变化数据后选择会报错的问题(< 1.12.13)

### 1.12.13-beta.4
- 修复 Select 下拉框弹出位置不正确的问题(< 1.12.13)
