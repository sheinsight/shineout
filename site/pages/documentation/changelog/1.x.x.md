# 更新日志
### 1.11.3
- 修复 Upload 组件在 Form.Item 中可能抛错的问题并优化相关校验逻辑
- 修复 Table 组件开启虚拟列表后在部分设备下滚动列表时可能导致浏览器前进或后退的问题
- 修复 Table 组件 loading 属性类型错误的问题
- 修复 Select 组件默认情况下允许输入的问题
- 修复 Select 组件通过 renderItem 传入空标签时可能导致 onFilter 无法输入的问题
- 修复 Select 组件开启 compressed 情况下使用 TreeData 时可能出现展示异常的问题
