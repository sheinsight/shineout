...
const style = {
  width: 100, textAlign: 'center', lineHeight: '30px', display: 'inline-block', border: 'solid 1px #eee',
}
    ...
    <div key={i}>
      {
        row.map((p, j) =>
        (p ? (
          <Popover style={{ margin: 4 }} content={content} position={p} key={j}>
            <div style={style}>{p}</div>
          </Popover>
          )
         : <div key={j} style={{ ...style, border: 0, margin: 4 }} />))
      }
    </div>
  ))
}
