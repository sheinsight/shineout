...
const examples = [
  ...
  {
    name: '02-format',
    title: locate('格式化 \n 传入值可为 日期对象，时间戳，字符串，通过format 属性可以定义返回值的格式', 'Format \n The incoming value can be a date object, a timestamp, a string, and the format of the return value can be defined by the format property.'),
  ...
  {
    name: '09-range-date',
    title: locate('选择范围 \n 设置 range 属性可以选择范围，输入和返回的 value 为长度为 2 的数组', 'Range \n Set the range property to select range and the input value and return value is an array of length 2.'),
