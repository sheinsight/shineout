...
const api = '[{"title":"DatePicker","properties":[{"name":"value","tag":{"en":"When the value is string, it needs to match the format attribute.\\n When the range property is true, the value is an array of length 2.","cn":"值为 string 时，需要和 format 属性匹配。\\n 非 string 会格式化为 string。\\n range 属性为 true 时，值为长度为2的数组"},"type":"string | number | Date | (Date | number | string )[]"},{"name":"onChange","tag":{"en":"a callback when the value is changing","cn":"值改变回调函数"},"type":"(value: Value, quickSelect?: { name: string value: Value } | null ) => void"},{"name":"beforeChange","tag":{"en":"The callback before the value is changed, when the return value is not empty, it will be used as the new value of the component","cn":"值改变前的回调，当返回值不为空时将作为组件的新值","override":"(value: any , datum?: FormDatum) => any"},"type":"(value: any , datum?: FormDatum) => any"},{"name":"onError","tag":{"en":"rules validation callback","cn":"rules 校验回调"},"type":"(e?: Error ) => void"},{"name":"popover","tag":{"en":"The position where the text pop up","cn":"信息弹出位置","override":"PopoverProps[\\\"position\\\"]"},"type":"PopoverProps[\\\"position\\\"]"},{"name":"disabled","tag":{"en":"When the value is true, disabled all options; When the value is function, disable the options that this function returns true.","cn":"如果 disabled 为 true，禁用全部选项，如果 disabled 为函数，根据函数反回结果禁用选项","default":"false"},"type":"boolean | ((date: Date, type?: \\\"start\\\" | \\\"end\\\" , value0?: Date , value1?: Date ) => boolean)"},{"name":"filterSameChange","tag":{"en":"onChange is not triggered when two selected values are the same","cn":"当两次选择的值相同时不触发onChange","default":"false"},"type":"boolean"},{"name":"bind","tag":{"en":"When the value changes, it will link to verify the fields in the bind, which needs to be used with Form","cn":"当值改变是会联动校验 bind 中的字段, 需要配合Form 使用"},"type":"string[]"},{"name":"defaultValue","tag":{"en":"defaultValue","cn":"默认值","override":"any"},"type":"any"},{"name":"reserveAble","tag":{"en":"If set to true, the form will not automatically delete the data after the component is uninstalled","cn":"设置为true 组件卸载后表单不自动删除数据"},"type":"boolean"},{"name":"rules","tag":{"en":"Validation rules, see Rule usage for details","cn":"校验规则 详见 Rule 用法","override":"RuleItem[]"},"type":"RuleItem[]"},{"name":"name","tag":{"en":"Form field, used with Form","cn":"表单字段,配合 Form 使用"},"type":"string | string[]"},{"name":"size","tag":{"en":"There are three built-in size: small、default、large.","cn":"不同尺寸","override":"union"},"type":"\\\"small\\\" | \\\"default\\\" | \\\"large\\\""},{"name":"onFocus","tag":{"en":"focus event callback","cn":"focus 事件回调"},"type":"(e: any) => void"},{"name":"onBlur","tag":{"en":"blur event callback","cn":"blur 事件回调"},"type":"(e: any) => void"},{"name":"tip","tag":{"en":"Prompt information","cn":"提示信息"},"type":"ReactNode"},{"name":"width","tag":{"en":"input width","cn":"输入框宽度"},"type":"string | number"},{"name":"popoverProps","tag":{"en":"Vilidate popup properties, specific properties refer to Popover component description","cn":"校验弹框接受的属性，具体属性参考Popover组件说明","type":""},"type":"PopoverProps"},{"name":"underline","tag":{"en":"only display border bottom","cn":"是否只展示下边框","default":"false"},"type":"boolean"},{"name":"border","tag":{"en":"Whether to display border","cn":"是否展示边框","default":"false"},"type":"boolean"},{"name":"className","tag":{"en":"extend className","cn":"扩展className"},"type":"string"},{"name":"style","tag":{"en":"style object","cn":"内联样式"},"type":"CSSProperties"},{"name":"format","tag":{"en":"default values for different types: \\\"date\\\": \\\"yyyy-MM-dd\\\". \\\"time\\\": \\\"HH:mm:ss\\\". \\\"week\\\": \\\"RRRR II\\\". \\\"month\\\": \\\"yyyy-MM\\\". \\\"week\\\": \\\"RRRR II\\\". \\\"quarter\\\": \\\"yyyy-[Q]Q\\\". \\\"year\\\": \\\"yyyy\\\". \\\"datetime\\\": \\\"yyyy-MM-dd HH:mm:ss\\\"","cn":"不同type对应的默认值。\\\"date\\\": \\\"yyyy-MM-dd\\\"。\\\"time\\\": \\\"HH:mm:ss\\\"。\\\"week\\\": \\\"RRRR II\\\"。\\\"month\\\": \\\"yyyy-MM\\\"。\\\"quarter\\\": \\\"yyyy-[Q]Q\\\"。 \\\"year\\\": \\\"yyyy\\\" 。 \\\"datetime\\\": \\\"yyyy-MM-dd HH:mm:ss\\\""},"type":"string"},{"name":"formatResult","tag":{"en":"Format the selected time","cn":"对选中时间进行格式化","default":"props.format"},"type":"string | ((date: Date) => string)"},{"name":"inputable","tag":{"en":"Allow enter something into DatePicker","cn":"可输入"},"type":"boolean"},{"name":"placeholder","tag":{"en":"placeholder text. When the range property is not empty, it is an array of length 2.","cn":"占位文字。range 属性不为空时，为长度为2的数组"},"type":"string | string[]"},{"name":"position","tag":{"en":"Set Position can control the different position of DatePicker","cn":"弹出框位置"},"type":"\\\"left-top\\\" | \\\"left-bottom\\\" | \\\"right-top\\\" | \\\"right-bottom\\\""},{"name":"range","tag":{"en":"range span，unit: **second**，When it is true, selection scope is not limited.","cn":"范围跨度，单位 **秒**，为 true 时表示不限制选择范围。"},"type":"number | boolean"},{"name":"type","tag":{"en":"type of datepicker","cn":"时间类型","default":"\\\"date\\\""},"type":"\\\"date\\\" | \\\"time\\\" | \\\"datetime\\\" | \\\"month\\\" | \\\"week\\\" | \\\"quarter\\\" | \\\"year\\\""},{"name":"allowSingle","tag":{"en":"allow single select, only in range can set","cn":"是否允许单选, 仅在 range 模式下有效","default":"false"},"type":"boolean"},{"name":"defaultTime","tag":{"en":"Default time when selecting a date, the format is: \\\"HH:mm:ss\\\"","cn":"选择日期时默认的时间, 格式为: \\\"HH:mm:ss\\\""},"type":"string | number | Date | (Date | number | string )[]"},{"name":"absolute","tag":{"en":"When it is true, the pop-up layer of option append into document.body; When it is a function, the return value is used as the popup layer container","cn":"为 true 时，选项弹出层在 DOM 中独立 render; 为函数时，返回值作为弹出层容器","default":"false"},"type":"boolean | (() => HTMLElement)"},{"name":"zIndex","tag":{"en":"options z-index should use with absolute","cn":"选项列表 z-index 值, 需要配合 absolute","default":"1000"},"type":"number"},{"name":"quickSelect","tag":{"en":"quick select, only in range can set, name: tip, value: range date","cn":"快速选择, 仅在 range 模式下有效, name: 文字提示, value: 时间范围","default":"false","override":"{name: string, value: \\\"}"},"type":"{name: string, value: \\\"}"},{"name":"min","tag":{"en":"option min value","cn":"可选最小值"},"type":"string | number | Date"},{"name":"max","tag":{"en":"option max value","cn":"可选最大值"},"type":"string | number | Date"},{"name":"defaultRangeMonth","tag":{"en":"The initial month of range selection, the value is a time object, valid only in range mode, and the priority is lower than value and defaultValue","cn":"范围选择的初始月份, 值为时间对象 或者时间戳, 仅在 range 模式下生效, 优先级低于 value 和 defaultValue"},"type":"(Date | number | string )[]"},{"name":"defaultPickerValue","tag":{"en":"default date of panel，work under has no value","cn":"面板默认时间，在未选择日期时生效"},"type":"string | number | Date | (Date | number | string )[]"},{"name":"hourStep","tag":{"en":"hour step","cn":"小时选项步长"},"type":"number"},{"name":"minuteStep","tag":{"en":"minute step","cn":"分钟选项步长"},"type":"number"},{"name":"secondStep","tag":{"en":"second step","cn":"秒选项步长"},"type":"number"},{"name":"onPickerChange","tag":{"en":"value onchange callback (every type of date)","cn":"值改变回调，有别于 onChange, onPickerChange会在每项值改变的时候执行"},"type":"(value: DateTimeType | DateTimeType[], quickSelect: void | { name: string value: Value } , areaType: \\\"year\\\" | \\\"month\\\" | \\\"week\\\" | \\\"day\\\" | \\\"time\\\" | \\\"quick\\\" | \\\"quarter\\\") => void"},{"name":"disabledTime","tag":{"en":"Disable the specified Time.","cn":"禁用指定 Time。"},"type":"string | ((time: string) => boolean)"},{"name":"align","tag":{"en":"horizontal align of the value","cn":"值水平排布方式","default":"center"},"type":"\\\"left\\\" | \\\"right\\\" | \\\"center\\\""},{"name":"clearable","tag":{"cn":"If clearable is true, show clear value icon","en":"是否显示清除数据图标","default":"true"},"type":"boolean"},{"name":"clearWithUndefined","tag":{"cn":"onChange get undefined while clear","en":"清空值时抛出 undefined","default":"false"},"type":"boolean"},{"name":"innerTitle","tag":{"en":"inner title","cn":"内嵌标题"},"type":"ReactNode"},{"name":"timeZone","tag":{"en":"Set the default time zone, the format is /^([+-]\\\\d{2})$/ Support \\\"-12\\\" to \\\"+13\\\"","cn":"设置默认时区,格式为/^([+-]\\\\d{2})$/ 支持 \\\"-12\\\" 到 \\\"+13\\\""},"type":"string"}]}]';
