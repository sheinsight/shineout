...
const api = '[{"title":"Select","properties":[{"name":"size","tag":{"cn":"尺寸","en":"size of select","default":"\\\"default\\\"","version":""},"type":"\\\"default\\\" | \\\"small\\\" | \\\"large\\\""},{"name":"style","tag":{"cn":"最外层扩展样式","en":"Container element style","default":"","version":""},"type":"CSSProperties "},{"name":"onCollapse","tag":{"cn":"下拉列表展开/收起回调","en":"option list collapse callback","default":"","version":""},"type":"((collapse: boolean) => void) "},{"name":"className","tag":{"cn":"扩展className","en":"extend className","default":"","version":""},"type":"string "},{"name":"placeholder","tag":{"cn":"默认占位内容 placeholder","en":"select default content","default":"","version":""},"type":"ReactNode"},{"name":"disabled","tag":{"cn":"如果 disabled 为 true，禁用全部选项，如果 disabled 为函数，根据函数反回结果禁用选项","en":"When the value is true, disabled all checkboxes; When the value is function, disable the checkbox that this function returns true.","default":"false","version":""},"type":"boolean | ((data: Item) => boolean) "},{"name":"value","tag":{"cn":"在Form中，value会被表单接管，value无效","en":"In the Form, the value will be taken over by the form and the value will be invalid.","default":"","version":""},"type":"any"},{"name":"onChange","tag":{"cn":"value 为 datum.getValue()","en":"value is the datum.getValue().","default":"","version":""},"type":"(value: Value, data?: Item , checked?: boolean ) => void"},{"name":"beforeChange","tag":{"cn":"值改变前的回调，当返回值不为空时将作为组件的新值","en":"The callback before the value is changed, when the return value is not empty, it will be used as the new value of the component","default":"","version":""},"type":"(value: any , datum?: FormDatum) => any"},{"name":"onError","tag":{"cn":"rules 校验回调","en":"rules validation callback","default":"","version":""},"type":"((e?: Error ) => void) "},{"name":"popover","tag":{"cn":"信息弹出位置","en":"The position where the text pop up","default":"","version":""},"type":"PopoverProps[\\\"position\\\"]"},{"name":"filterSameChange","tag":{"cn":"过滤掉相同值的回调","en":"filter out value change callbacks with the same value","default":"false","version":""},"type":"boolean "},{"name":"bind","tag":{"cn":"当值改变是会联动校验 bind 中的字段, 需要配合Form 使用","en":"When the value changes, it will link to verify the fields in the bind, which needs to be used with Form","default":"","version":""},"type":"string[] "},{"name":"defaultValue","tag":{"cn":"默认值","en":"Initial value","default":"","version":""},"type":"any"},{"name":"reserveAble","tag":{"cn":"设置为true 组件卸载后表单不自动删除数据","en":"If set to true, the form will not automatically delete the data after the component is uninstalled","default":"","version":""},"type":"boolean "},{"name":"rules","tag":{"cn":"校验规则 详见 Rule 用法","en":"Validation rules, see Rule usage for details","default":"","version":""},"type":"RuleItem[]"},{"name":"name","tag":{"cn":"表单字段,配合 Form 使用","en":"Form field, used with Form","default":"","version":""},"type":"string | string[] "},{"name":"loading","tag":{"cn":"数据加载中，为true时会展示一个默认的[Spin](/components/Spin)组件，可以传入一个自定义的Spin代替","en":"When it is true, a default [Spin](/components/Spin) component will be displayed, a custom loading icon can be passed in to replace.","default":"false","version":""},"type":"ReactNode"},{"name":"separator","tag":{"cn":"多选情况下设置后，value 会处理为 separator 分隔的字符串。","en":"set with multiple, value will separator by this","default":"","version":""},"type":"string "},{"name":"prediction","tag":{"cn":"(val, d) => val===format(d) | 默认使用 format 函数执行的结果来比较是否匹配，在某些情况下（例如返回原始数据的对象，更新数据时，生成了一个值相同，非同一个对象的选项），需要借助 prediction 函数来判断是否匹配","en":"By default, the result of the format function is used to compare whether it matches. In some cases (for example, whe an object that returns the original data is updated, an different option with the same value  is generated), the prediction function needs to be used to determine whether match","default":"(val, d) => val===format(d)","version":""},"type":"((value: Value extends (infer U)[] ? U : Value, data: Item) => boolean) "},{"name":"format","tag":{"cn":"格式化 value。 默认值，返回原始数据。 为string时，会作为key从原始数据中获取值，相当于 (d) => d[format]。为函数时，以函数返回结果作为 value。","en":"Format value. The defaule value is return the original data. When it is a string, the value is fetched from the original data as a key equivalent to (d) => d[format] When it is a function, use its return value.","default":"d => d","version":""},"type":"ObjectKey<Item> | ((data: Item) => Value extends (infer U)[] ? U : Value)"},{"name":"renderItem","tag":{"cn":"为 string 时，返回 d[string]。 为 function 时，返回函数结果","en":"When it is a string, return d[string]. When it is a function, return the result of the function.","default":"","version":""},"type":"((data: Item, index?: number ) => ReactNode) | ObjectKey<Item> "},{"name":"keygen","tag":{"cn":"生成key的辅助方法, 为函数时，使用此函数返回值, 为string时，使用这个string对应的数据值。如 \\\"id\\\"，相当于 (d) => d.id","en":"Auxiliary method for generating key. When it is a function, use the return value of this function. When it is a string, use the data value corresponding to this string. For example, \\\"id\\\" is the same thing as (d) => d.id.","default":"index","version":""},"type":"ObjectKey<DataItem> | ((data: DataItem, index?: number) => KeygenResult) | true"},{"name":"data","tag":{"cn":"筛选后是否展示命中节点的后代节点","en":"Options data","default":"","version":""},"type":"Item[] "},{"name":"onFocus","tag":{"cn":"focus 回调","en":"focus callback","default":"","version":""},"type":"(e?: any) => void"},{"name":"onBlur","tag":{"cn":"blur 回调","en":"blur callback","default":"","version":""},"type":"(e?: any) => void"},{"name":"tip","tag":{"cn":"提示信息","en":"Prompt information","default":"","version":""},"type":"ReactNode"},{"name":"width","tag":{"cn":"输入框宽度","en":"input width","default":"","version":""},"type":"string | number "},{"name":"popoverProps","tag":{"cn":"校验弹框接受的属性，具体属性参考Popover组件说明","en":"Vilidate popup properties, specific properties refer to Popover component description","default":"","version":""},"type":"PopoverProps "},{"name":"underline","tag":{"cn":"仅仅展示下边框","en":"show border bottom","default":"false","version":""},"type":"boolean "},{"name":"autoFocus","tag":{"cn":"是否自动获得焦点","en":"Whether to automatically get the focus","default":"false","version":""},"type":"boolean "},{"name":"border","tag":{"cn":"是否展示边框","en":"Whether to display border","default":"false","version":""},"type":"boolean "},{"name":"position","tag":{"cn":"弹出层位置","en":"The position of the pop-up layer, options: [\\\"left\\\", \\\"top\\\", \\\"right\\\", \\\"bottom\\\"]","default":"auto","version":""},"type":".ListPosition "},{"name":"absolute","tag":{"cn":"为 true 时，选项弹出层在 DOM 中独立 render; 为函数时，返回值作为弹出层容器","en":"When it is true, the pop-up layer of option append into document.body; When it is a function, the return value is used as the popup layer container","default":"false","version":""},"type":"boolean | (() => HTMLElement) "},{"name":"zIndex","tag":{"cn":"选项列表 z-index 值, 需要配合 absolute","en":"options z-index should use with absolute","default":"1000","version":""},"type":"number "},{"name":"clearable","tag":{"cn":"是否可清除值","en":"If clearable is true, show clear value icon","default":"false","version":""},"type":"boolean "},{"name":"innerTitle","tag":{"cn":"内嵌标题","en":"inner title","default":"","version":""},"type":"ReactNode"},{"name":"open","tag":{"cn":"控制浮层显隐","en":"Set visible of select popup","default":"","version":""},"type":"boolean "},{"name":"showArrow","tag":{"cn":"是否显示下拉箭头，仅针对单选情况","en":"show dropdown arrow, only single select","default":"true","version":""},"type":"boolean "},{"name":"renderResult","tag":{"cn":"为 string 时，返回 d[string]。为 function 时，返回函数结果","en":"When it is a string, return d[string]. When it is a function, return the result of the function.","default":"(val, d) => val===format(d)","version":""},"type":"((data: Item, index?: number ) => ReactNode) "},{"name":"trim","tag":{"cn":"trim 为 true 时，失去焦点时会自动删除空白字符。","en":"When trim is true, blank characters are automatically deleted when lose focus。","default":"false","version":""},"type":"boolean "},{"name":"maxLength","tag":{"cn":"Select 输入框输入字符串最大长度","en":"The maximum length of the input string in the Select input box","default":"","version":""},"type":"number "},{"name":"height","tag":{"cn":"高度","en":"height","default":"250","version":""},"type":"number "},{"name":"lineHeight","tag":{"cn":"选项高度。列表项使用虚拟列表渲染，当选项高度改变时，应该通过 lineHeight 来指定正确高度","en":"Option height. List items are rendered using virtual lists, and when the option height changes, the correct height should be specified via lineHeight","default":"34","version":""},"type":"number "},{"name":"onAdvancedFilter","tag":{"cn":"使用 onAdvancedFilter 属性开启高级筛选，可针对当前层级在筛选结果和原始数据间切换","en":"Use the onAdvancedFilter property to enable filtering to switch between filtering results and raw data for the current hierarchy.","default":"","version":""},"type":"((text: string) => void | ((data: Item) => boolean)) "},{"name":"onFilter","tag":{"cn":"onFilter 不为空时，可以输入过滤数据。onFilter 如果返回一个函数，使用这个函数做前端过滤。如果不返回，可以自行做后端过滤","en":"When the onFilter is not empty, you can filter data by input. If the onFilter returns a function, use this function as a front-end filter. If return undefined, you can do your own backend filtering.","default":"","version":""},"type":"((text: string, from?: string ) => void | ((data: Item) => boolean) ) "},{"name":"onExpand","tag":{"cn":"节点展开回调，参数为当前展开节点 key 数组","en":"expand event","default":"","version":""},"type":"((value: (string | number)[]) => void) "},{"name":"expanded","tag":{"cn":"展开的节点 key （受控）","en":"expanded node","default":"","version":""},"type":"(string | number)[] "},{"name":"defaultExpanded","tag":{"cn":"默认展开的节点 key（非受控）","en":"default expanded nodes","default":"","version":""},"type":"(string | number)[] "},{"name":"defaultExpandAll","tag":{"cn":"默认展开全部子节点, 仅树形数据下有效","en":"expand all node, only in can be use in treeData","default":"false","version":""},"type":"boolean "},{"name":"loader","tag":{"cn":"设置loader属性后，未定义children的节点视为动态加载节点，点击展开触发 loader事件，children 为 null 或者长度为 0 视为叶子节点","en":"dynamically load nodes","default":"","version":""},"type":"((key: Value extends (infer U)[] ? U : Value, data: DataItem) => void) "},{"name":"childrenKey","tag":{"cn":"指定子数据的属性名","en":"specify the name of the subdata","default":"children","version":""},"type":"ObjectKey<DataItem> "},{"name":"columns","tag":{"cn":"columns 大于 1 时，选项展示为多列布局模式","en":"Option columns.","default":"1","version":""},"type":"number "},{"name":"treeData","tag":{"cn":"树形结构数据项，[{children: []}]","en":"tree select data，[{children: []}]","default":"","version":""},"type":"object[]"},{"name":"itemsInView","tag":{"cn":"单次render的最大行数。Select 采用了lazy render的方式来优化在大量数据下的性能，如果你的表格显示的高度超出了10条，可以调整itemsInView","en":"The maximum number of rows for a single render. Select uses lazy render to optimize performance under large amounts of data. If your table displays more than 10 rows, you can change the value of itemsInView.","default":"10","version":""},"type":"number "},{"name":"multiple","tag":{"cn":"是否是多选","en":"if it is true, it will be multiple selection","default":"false","version":""},"type":"boolean "},{"name":"onCreate","tag":{"cn":"如果设置了 onCreate 事件，组件为可输入状态。onCreate为函数时，将此函数返回值作为新的选项拆入最上方。onCreate为true时，使用默认函数 text => text","en":"If the onCreate event is set, the component is inputable. When onCreate is a function, the return value of this function is diaplay at the top as a new option. When onCreate is true, use the built-in functuon text => text.","default":"","version":""},"type":"boolean | ((input: string | Item) => string | Item) "},{"name":"compressed","tag":{"cn":"将选中值合并，只在多选模式下有效","en":"Merges selected values, valid only in multiselect mode","default":"false","version":""},"type":"boolean | \\\"no-repeat\\\" "},{"name":"compressedBound","tag":{"cn":"开启多选后，指定允许展示标签数量，超过后将折叠","en":"when compressed is True,the comptessedBound can limit the numbers of multiple selected item\\\"s label","default":"","version":""},"type":"number "},{"name":"autoAdapt","tag":{"cn":"下拉列表宽度根据内容自由展开","en":"option list is auto adapt","default":"false","version":""},"type":"boolean "},{"name":"filterSingleSelect","tag":{"cn":"当筛选数据仅为一条时，失焦后直接选中该条数据。仅在 Filter 下有效。","en":"blur to select the data when filter data has only single. only work in filter.","default":"false","version":""},"type":"boolean "},{"name":"renderUnmatched","tag":{"cn":"渲染未匹配值的方式","en":"the way to render not matched data value","default":"","version":""},"type":"((data: Value extends (infer U)[] ? U : Value) => ReactNode) "},{"name":"emptyAfterSelect","tag":{"cn":"选中后是否清空输入框内容","en":"empty input after select value","default":"false","version":""},"type":"boolean "},{"name":"focusSelected","tag":{"cn":"onCreate 或 onFilter 在单选情况下单击值后是否选中值","en":"selected value while click under onCreate or onFilter","default":"true","version":""},"type":"boolean "},{"name":"compressedClassName","tag":{"cn":"多选合并展示弹出框的类名","en":"compressed popover classname","default":"","version":""},"type":"string "},{"name":"resultClassName","tag":{"cn":"选中结果内容容器的className","en":"The className of the selected result content container","default":"","version":""},"type":"string | ((value: Item) => string) "},{"name":"reFocus","tag":{"cn":"存在onFilter和onCreate，选中 Option，自动focus Input","en":"There are onFilter and onCreate, select Option, automatically focus Input","default":"false","version":""},"type":"boolean "},{"name":"header","tag":{"cn":"自定义渲染 Option List Header","en":"Custom render option list header","default":"null","version":""},"type":"ReactElement "},{"name":"convertBr","tag":{"cn":"用来转化粘贴文本中的换行","en":"Used to convert line breaks in pasted text","default":"\\\",\\\"","version":""},"type":"string | ((text: string) => string) "},{"name":"renderOptionList","tag":{"cn":"自定义渲染下拉列表","en":"Custom render dropdown","default":"","version":""},"type":"((list: ReactElement, info: { loading: boolean; }) => ReactElement) "},{"name":"onEnterExpand","tag":{"cn":"回车触发下拉框展开的时候调用","en":"expand option list while enter press","default":"","version":""},"type":"((e: KeyboardEvent<HTMLDivElement>) => boolean) "},{"name":"hideCreateOption","tag":{"cn":"在使用创建选项时，在选项列表中隐藏该选项，回车后直接选中","en":"hide the creat option while set onCreate","default":"false","version":""},"type":"boolean "},{"name":"optionWidth","tag":{"cn":"下拉列表宽度","en":"width of option list","default":"100%","version":""},"type":"number "},{"name":"columnWidth","tag":{"cn":"columns 大于 1 时，选项展示为多列布局模式","en":"Option column width, only effective when columns > 1","default":"160","version":""},"type":"number "},{"name":"groupBy","tag":{"cn":"分组","en":"group by","default":"","version":""},"type":"((record: Item, index: number, data: Item[]) => string) "},{"name":"columnsTitle","tag":{"cn":"多列选项多选时的标题文字","en":"title of columns multiple select","default":"","version":""},"type":"ReactNode"},{"name":"emptyText","tag":{"cn":"自定义 empty 文案","en":"custom empty copy","default":"","version":""},"type":"ReactNode"},{"name":"filterDelay","tag":{"cn":"毫秒。用户输入触发 fitler 事件的延时","en":"ms. The delay of user input triggering filter events","default":"400","version":""},"type":"number "},{"name":"showHitDescendants","tag":{"cn":"筛选后是否展示命中节点的后代节点","en":"Whether to show the descendant nodes of the hit node after filtering","default":"false","version":""},"type":"boolean "},{"name":"noCache","tag":{"cn":"是否开启数据缓存，如果数据存在动态更新的情况建议开启","en":"data cache, if data change asynchronously, better set true","default":"false","version":""},"type":"boolean "}],"cn":"","en":""}]';
