...
const api = '[{"title":"TableColumn","properties":[{"name":"align","tag":{"cn":"单元格内容排布方式，可选 \\[\\\"left\\\", \\\"center\\\", \\\"right\\\"\\]","en":"cell align \\[\\\"left\\\", \\\"center\\\", \\\"right\\\"\\]","default":"\\\"left\\\"","version":""},"type":".Align "},{"name":"colSpan","tag":{"cn":"合并列控制函数，row为单行数据，返回值一个整数，标明需要合并的列数","en":"The function for controlling to merge columns. The return value is an integer indicating the number of columns that need to be merged。","default":"","version":""},"type":"((row: T, index: number) => number) "},{"name":"defaultOrder","tag":{"cn":"默认排序规则","en":"default sort","default":"","version":""},"type":"\\\"asc\\\" | \\\"desc\\\" "},{"name":"fixed","tag":{"cn":"可选\\[\\\"left\\\", \\\"right\\\"]；需要设置Table的fixed为\\\"x\\\"或\\\"both\\\"才生效；如果相邻的多列需要锁定，只需指定最外侧的column即可","en":"options: \\[\\\"left\\\", \\\"right\\\"]；Need to set the Table\\\"s fixed to \\\"x\\\" or \\\"both\\\"","default":"","version":""},"type":"\\\"left\\\" | \\\"right\\\" "},{"name":"group","tag":{"cn":"表头分组，相邻的相同 group 会生成一个新的表头","en":"The group of header column.","default":"","version":""},"type":"ReactNode | ReactNode[]"},{"name":"hide","tag":{"cn":"只针对行展开列有效，表示是否隐藏该列","en":"hide the column, only work on row-expand column","default":"","version":""},"type":"boolean "},{"name":"key","tag":{"cn":"列的key，默认使用index","en":"The key of the column","default":"","version":""},"type":"string | number "},{"name":"minWidth","tag":{"cn":"最小列宽","en":"min width","default":"","version":""},"type":"number "},{"name":"maxWidth","tag":{"cn":"最大可拖动列宽","en":"max width","default":"","version":""},"type":"number "},{"name":"filterAll","tag":{"cn":"最大可拖动列宽","en":"全选时用来筛除数据，仅当type=\\\"checkbox\\\"的时候有效","default":"","version":""},"type":"((data: T[]) => T[]) "},{"name":"render","tag":{"cn":"表格内容生成函数；d: 当前行数据i: 当前行索引为了使用方便，可以传入一个数据的key，如 \\\"id\\\"，相当于 (d) => { return d.id }","en":"The generation function for Table content.d: the data of the current row. i: the index of the current row .For ease of use, you can pass in the key of a data, such as \\\"id\\\", which is equivalent to (d) => { return d.id }","default":"","version":""},"type":"ObjectKey<T> | ((rowData: T, index: number, checkInstance?: ReactComponentElement<any, Pick<any, string | number | symbol>> ) => ReactNode | (() => ReactNode)) "},{"name":"rowSpan","tag":{"cn":"根据函数返回的结果（boolean）判断是否合并行，a、b为相邻的两行数据。","en":"According to the result (boolean) returned by the function to determine whether to merge rows, a and b are two adjacent rows of data","default":"","version":""},"type":"boolean | ((prevRowData: T, nextRowData: T) => boolean) "},{"name":"sorter","tag":{"cn":"sorter 不为空时，这一列会出现排序 icon。order的值为\\[\\\"asc\\\", \\\"desc\\\"]。字符串表示排序依据字段，作为第一个参数传入Table.sorter。前端排序，返回一个排序函数，参考 Array.sort。服务端排序，不要返回值，自行处理即可。","en":"When the sorter is not empty, the sort icon appears in this column. the value of order: \\[\\\"asc\\\", \\\"desc\\\"]. Indicate the sort key string, will pass to table sorter method. Front-end sorting returns a sort function, refer to Array.sort. Server-side sorting, do not return values and handle it itself.","default":"","version":""},"type":"string | ((order: \\\"asc\\\" | \\\"desc\\\") => void | ((prevRowData: T, nextRowData: T) => number)) | Sorter "},{"name":"title","tag":{"cn":"表头显示内容","en":"The content of the header","default":"","version":""},"type":"ReactNode | ((rowData: T[]) => ReactNode)"},{"name":"treeColumnsName","tag":{"cn":"树形表格子数据字段名","en":"tree table children-data name","default":"","version":""},"type":"ObjectKey<T> "},{"name":"treeIndent","tag":{"cn":"每一层缩进宽度","en":"indent of each level","default":"","version":""},"type":"number "},{"name":"type","tag":{"cn":"特殊用途列，可选值为 \\[\\\"expand\\\", \\\"row-expand\\\", \\\"checkbox\\\"]。expand: 行展开列，render 函数返回函数时，表示此行可以展开，内容为此函数返回结果。row-expand: 同expand。不同为点击行内空白区域也可以折叠/展开行。checkbox: 选择列，用于仅固定选择列的场景","en":"Special column, options: \\[\\\"expand\\\", \\\"row-expand\\\", \\\"checkbox\\\"].expand: Expand the column. When the render function returns a function, it means that the row can be expanded and the content  is the result returned by this function. row-expand: Similar to expand. The difference is that clicking on the entire row triggers the expand event. checkbox: Select column for scenes with only fixed selection columns","default":"","version":""},"type":"\\\"expand\\\" | \\\"row-expand\\\" | \\\"checkbox\\\" "},{"name":"width","tag":{"cn":"列宽","en":"width","default":"","version":""},"type":"number "},{"name":"className","tag":{"cn":"列对应的类名","en":"classname of column","default":"","version":""},"type":"string "},{"name":"style","tag":{"cn":"td 样式","en":"style of td","default":"","version":""},"type":"CSSProperties "},{"name":"onClick","tag":{"cn":"列点击事件","en":"Click event of column","default":"","version":""},"type":"((d: T, isExpand: boolean) => void) "},{"name":"columnResizable","tag":{"cn":"单独设置某一列不可拖动","en":"Separately set a column not to be draggable","default":"","version":""},"type":"false "}],"cn":"","en":""},{"title":"Table","properties":[{"name":"onRowSelect","tag":{"cn":"选择行。rows为选中的数据。如果需要数据需要格式化的处理，建议配置 format 和 prediction","en":"Select row. Rows is the selected data.","default":"","version":""},"type":"((rows: Value) => void) "},{"name":"sorter","tag":{"cn":"表格统一排序函数，参数分别为 Column.sorter 和 排序方式; 支持多列排序，sorter传入对象{ rule: string \\| function, weight: number }, rule为排序规则，为字符串时参考单列排序的用法, weight为权重，指明排序的优先级。 多列排序时，sortedList返回所有参与排序的字段信息","en":"the method of table sort，args are Column.sorter and order. Multi-column sorting is supported. The sorter passes in the object {rule: string | function, weight: number}, where rule is a sorting rule, which refers to the use of single-column sorting when it is a string, weight is the weight, indicating the priority of the order. When sorting on multiple columns, sortedList returns information about all fields involved in sorting","default":"alphaSort(Column.sorter, sorter)","version":""},"type":"(sortKey: string, sorter: \\\"asc\\\" | \\\"desc\\\", sortedList: any[]) => void | ((a: DataItem, b: DataItem) => number)"},{"name":"onSortCancel","tag":{"cn":"排序取消事件","en":"sort cancel event","default":"","version":""},"type":"((prevType: \\\"asc\\\" | \\\"desc\\\", index: number, orders: any, sort: any) => void) "},{"name":"prediction","tag":{"cn":"(val, d) => val===format(d) | 默认使用 format 函数执行的结果来比较是否匹配，在某些情况下（例如返回原始数据的对象，更新数据时，生成了一个值相同，非同一个对象的选项），需要借助 prediction 函数来判断是否匹配","en":"By default, the result of the format function is used to compare whether it matches. In some cases (for example, whe an object that returns the original data is updated, an different option with the same value  is generated), the prediction function needs to be used to determine whether match","default":"(val, d) => val===format(d)","version":""},"type":"((value: Value extends (infer U)[] ? U : Value, data: DataItem) => boolean) "},{"name":"format","tag":{"cn":"格式化 value。 默认值，返回原始数据。 为string时，会作为key从原始数据中获取值，相当于 (d) => d[format]。为函数时，以函数返回结果作为 value。","en":"Format value. The defaule value is return the original data. When it is a string, the value is fetched from the original data as a key equivalent to (d) => d[format] When it is a function, use its return value.","default":"d => d","version":""},"type":"ObjectKey<Item> | ((data: Item) => Value extends (infer U)[] ? U : Value)"},{"name":"pagination","tag":{"cn":"展示分页 详见 Pagination","en":"Show pagination See Pagination for details","default":"","version":""},"type":"PaginationProps "},{"name":"summary","tag":{"cn":"底部信息可用于总结\\ndefault: -","en":"Footer information can be used to summarize","default":"","version":""},"type":"({ render: () => ReactNode colSpan?: number  rowSpan?: number  })[] | ({ render: () => ReactNode colSpan?: number  rowSpan?: number  })[][] "},{"name":"onTreeExpand","tag":{"cn":"展开行，keys为展开的行","en":"expand row change, keys is expanded row keys","default":"","version":""},"type":"((openKeys: (string | number)[], data: DataItem, expand: boolean, index: number) => void) "},{"name":"treeExpandKeys","tag":{"cn":"树形数据展开行，受控","en":"Tree Table expanded row keys","default":"","version":""},"type":"(string | number)[] "},{"name":"changedByExpand","tag":{"cn":"在特定场景（树形数据展开受控)下开启 用来改变滚动条重置的默认行为","en":"Enable in specific scenarios (tree data expansion is controlled) Used to change the default behavior of scroll reset","default":"false","version":""},"type":"boolean "},{"name":"innerScrollAttr","tag":{"cn":"虚拟滚动模式下，设置行内元素的 attribut 来实现内部滚动","en":"set inner scrollable element\\\"s attribute","default":"\\\"无\\\"","version":""},"type":"string[] "},{"name":"rowEvents","tag":{"cn":"tr 事件监听器集合","en":"tr events","default":"","version":""},"type":"{ [name: string]: V } "},{"name":"renderSorter","tag":{"cn":"自定义排序图标","en":"customize sort icons","default":"false","version":""},"type":"((params: { status?: \\\"asc\\\" | \\\"desc\\\"  triggerAsc: () => void triggerDesc: () => void }) => ReactNode) "},{"name":"sticky","tag":{"cn":"表头是否附着顶部，为 true 时距离顶部为0，为对象时属性值参考 [Sticky组件](/components/Sticky)","en":"sticky header, When it is true, the distance from the top is 0. When it is an object, the attribute value reference [Sticky component] (/components/Sticky)","default":"","version":""},"type":"boolean | StickyProps "},{"name":"tableRef","tag":{"cn":"Table 实例（请谨慎使用：仅固定表格）","en":"Table instance (please use with caution: only fixed Table)","default":"","version":""},"type":"((table: TableRef) => void) "},{"name":"verticalAlign","tag":{"cn":"单元格内容垂直对齐方式","en":"vertical align with content","default":"\\\"top\\\"","version":""},"type":"\\\"top\\\" | \\\"middle\\\" "},{"name":"cellSelectable","tag":{"cn":"是否启用 ctrl/cmd + click 选中单元格","en":"whether to enable ctrl/cmd + click check","default":"false","version":""},"type":"boolean "},{"name":"defaultTreeExpandKeys","tag":{"cn":"默认展开行(非受控)","en":"Default expanded row keys","default":"","version":""},"type":"(string | number)[] "}],"cn":"","en":""}]';
